...
export function cyclesReducer(state: CyclesState, action: any) {
  switch (action.type) {
    ...
    case ActionTypes.MARK_CURRENT_CYCLE_AS_FINISHED:
      ...
      const currentFinishedCycleIndex = state.cycles.findIndex(cycle => cycle.id === state.activeCycleId);
    ...
    case ActionTypes.INTERRUPT_CYCLE:
      const currentInterruptCycleIndex = state.cycles.findIndex(cycle => cycle.id === state.activeCycleId);
